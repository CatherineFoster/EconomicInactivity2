...
long_dta_combined_hiqual %>% 
  ...
  mutate(
    simplified_highest_qualification = case_when(
      ...
      highqual %in% c("GCSE etc", "Other qualification") ~ "GCSE or similar",
      highqual == "A-level etc" ~ "A-level etc",
...
extract_three_periods <- function(data, waves){
  data %>% 
    ...
    left_join(
      data %>% 
          filter(variable == "fihhmnnet3_dv") %>% 
          filter(wave == waves[1]) %>% 
          mutate(income = as.double(value)) %>% 
          select(pidp, income)  
    ) %>% 
...
BIC(mod_t2a2sq_T1, mod_t2a2s_T1)

```

This BIC comparison suggests the inclusion of all highest qualifications does not improve the pentalised model fit. Let's now look at the summary stats.

```{r}
summary(mod_t2a2sq_T1)
```

The summary stats suggest only the category of having no qualifications is significantly predictive of economic inactivity. Let's consider a model just with this binary variable:
...
BIC(mod_t2a2sq2_T1, mod_t2a2s_T1)
```
...
```{r}
summary(mod_t2a2sh_T1)
```

```{r}
BIC(mod_t2a2sh_T1, mod_t2a2s_T1)
```

As expected, adding many household categories worsens the penalised model fit.
