...
-   [ ] Update script to work with new helper functions

-   [ ] Identify revised results and produce way of porting these over more clearly
...
-   [ ] Add ethnicity to baseline model and those that develop from it

-   [ ] Make sure this is also added to other relevant docs
...
long_slimmed_data
 ...
 # Now trying to just extract ethnicity
 varnames <-  c(
   "ethn_dv"
   )
 ...
 long_slimmed_ethnic_datalist <- lapply(indresp_files, 
        read_and_slim_data, 
        varnames = varnames, 
        extract_what = extract_what, 
        verbose = TRUE
 ...
 long_slimmed_ethnicdata <- bind_rows(long_slimmed_ethnic_datalist)
 ...
 rm(long_slimmed_ethnic_datalist)
 
long_slimmed_ethnicdata
...
Now the same sort of thing for ethnicity

```{r}

ethnic_statuses <- 
  long_slimmed_data %>% 
    filter(variable == 'ethn_dv') %>% 
    select(pidp, wave, ethnicity_status = value)

ethnic_statuses
```

Let's try to reduce the number of categories

```{r}
ethnic_statuses %>% group_by(wave, ethnicity_status) %>% count() %>% 
  group_by(wave) %>% 
...
Most ethnic groups in the ukhls j wave seem broadly representative of what's observed in the 2021 census, though Chinese appears around half (or less) the reported rate

We now also have a slightly simplified taxonomy to use

```{r}

ethnicity_recodings <- readxl::read_excel(path = here("data/ethnicity_categories.xlsx"), sheet = 'regroupings') %>% 
  janitor::clean_names()

ethnic_statuses %>% 
  left_join(
    ethnicity_recodings, 
    by = c('ethnicity_status' = 'original')
  ) %>% 
  group_by(wave, recoded) %>% count() %>% 
  group_by(wave) %>% 
  mutate(share = n / sum(n)) %>% 
  filter(wave == 'j') %>% 
  arrange(desc(share)) %>% 
...
ethnicity_statuses_simplified <- 
  ethnic_statuses %>% 
  left_join(
    ethnicity_recodings,
    by = c('ethnicity_status' = 'original')
  ) %>% 
  select(pidp, wave, ethnicity_status = recoded) %>% 
  filter(ethnicity_status != 'Missing')
...
econ_act_current_gvn_last_with_sex_age_and_sf12 <- 
  ...
  left_join(
    ethnicity_statuses_simplified %>%
      mutate(wavenumber = match(wave, letters[1:26])) %>%
      select(pidp, wavenumber, ethnicity_status)
  ) %>%
...
mod_01a <- multinom( 
  this_status ~ last_status * sex + splines::bs(age, df = 5) + ethnicity_status, #b-splines with 5 dfs
  data = econ_act_current_gvn_last_with_sex_age_and_sf12
)



# The following model aimed to test interactions between ethnicity and sex, but could not be run with 
# the available data

# mod_01b <- multinom( 
#   this_status ~ last_status * sex * ethnicity_status + splines::bs(age, df = 5), #b-splines with 5 dfs
#   data = econ_act_current_gvn_last_with_sex_age_and_sf12
# )

mod_01c <- multinom( 
  this_status ~ last_status * sex + splines::bs(age, df = 5) + is_british, #b-splines with 5 dfs
  data = econ_act_current_gvn_last_with_sex_age_and_sf12 %>% 
    mutate(is_british = ethnicity_status == 'British') 
)

mod_01d <- multinom( 
  this_status ~ last_status * sex + splines::bs(age, df = 5), #b-splines with 5 dfs
  data = econ_act_current_gvn_last_with_sex_age_and_sf12 %>% filter(wavenumber <= 11)
)

mod_01e <- multinom( 
  this_status ~ last_status * sex + splines::bs(age, df = 5) + pandemic_year, #b-splines with 5 dfs
  data = econ_act_current_gvn_last_with_sex_age_and_sf12 %>% 
    mutate(pandemic_year = wavenumber >= 11)
)
...
Let's compare the two baseline models, one with and the other without ethnicity

```{r}

# model 01b was too complex to be calculable
BIC(mod_01, mod_01a, mod_01c)
AIC(mod_01, mod_01a, mod_01c)


# We also want to see if there's a signal that the pandemic year was sufficiently distinct from other years 
...
Example of getting predictions from our foundational model `mod_01`

```{r}
mod_01

vignette_data <- tribble(
    ~last_status, ~age, ~sex,
    "Employed", 30, "female",
    "Unemployed", 55, "male",
    "Unemployed", 55, "female",
    "Unemployed", 62, "male"
)

predict(
  mod_01, 
  newdata = vignette_data
)
...
actual_predictions <- predict(mod_01)
tmp <- 
  econ_act_current_gvn_last_with_sex_age_and_sf12 %>% 
    mutate(predicted_next_wave = actual_predictions) %>% 
  select(everything(), this_status, predicted_next_wave)

view(tmp)

xtabs(~predicted_next_wave + this_status, data = tmp)

```

## Another baseline example

Let's look at how age and gender affect the probability of remaining employed for persons who start off employed, and becoming employed for persons who start off unemployed

```{r}
demographic_predictor_matrix <- expand_grid(
  sex = c("male", "female"),
  age = 20:60,
  last_status = c("Unemployed", "Employed")
)

demographic_prediction_matrix <- predict(mod_01, newdata = demographic_predictor_matrix, type = "probs")


combined_matrix <- bind_cols(demographic_predictor_matrix, demographic_prediction_matrix) %>% 
  pivot_longer(cols = Employed:Unemployed, names_to = "predicted_state", values_to = "probability")

```

Now let's use this to show how age and gender affect the probabilities of the employed staying employed, and the unemployed becoming employed

Probability of employed remaining employed

```{r}
combined_matrix %>% 
  filter(last_status == "Employed") %>% 
  filter(predicted_state == "Employed") %>% 
  ggplot(aes(x=age, y= probability, colour = sex)) + 
  geom_line() + 
  labs(
    x = "Age in years", 
    y = "Probability of remaining employed", 
    title = "Predicted probabilities of employed persons remaining employed at next wave given age and sex"
  )
...
combined_matrix %>% 
  ...
  labs(
    x = "Age in years", 
    y = "Probability of becoming employed", 
    title = "Predicted probabilities of unemployed persons becoming employed at next wave given age and sex"
  )


combined_matrix %>% 
  filter(last_status == "Unemployed") %>% 
  filter(predicted_state == "Inactive long term sick") %>% 
  ggplot(aes(x=age, y= probability, colour = sex)) + 
  geom_line() + 
  expand_limits(y = 0) +
  labs(
    x = "Age in years", 
    y = "Probability of becoming becoming long-term sick", 
    title = "Predicted probabilities of unemployed persons becoming long-term sick at next wave given age and sex"
  )
...
predictions_control_mh_wave_j <- 
  predict(mod_02, newdata = predictor_control_mh_wave_j, type = 'probs')

predictions_treatment_mh_wave_j <- 
  predict(mod_02, newdata = predictor_treatment_mh_wave_j, type = 'probs')

```

```{r}
predictions_predictors_control_mh_wave_j <- 
  bind_cols(
    predictor_control_mh_wave_j, predictions_control_mh_wave_j
  ) %>% 
...
predictions_predictors_both_mh_wave_j <- 
  bind_rows(
    predictions_predictors_control_mh_wave_j,
    predictions_predictors_treatment_mh_wave_j
  )
...
Let's now extract the summary distributions in both arms

```{r}
summary_status_pool_estimated_mh_wave_j <- 
  predictions_predictors_both_mh_wave_j %>% 
    group_by(arm, current_status) %>% 
    summarise(total_estimated_size = sum(predicted_probability)) %>% 
    ungroup() %>% 
    group_by(arm) %>% 
    mutate(share_of_population_in_state = total_estimated_size / sum(total_estimated_size)) %>% 
    pivot_wider(names_from = arm, values_from = c(total_estimated_size, share_of_population_in_state)) %>% 
    select(current_status, total_estimated_size_control, share_of_population_in_state_control, total_estimated_size_treatment, share_of_population_in_state_treatment)

write_csv(x = summary_status_pool_estimated_mh_wave_j, file = "summary_status_pool_estimated_wave_j.csv")
summary_status_pool_estimated_mh_wave_j
...
Let's now round this off by doing the same analysis but for wave 10
...
predictions_control_ph_wave_j <- 
  predict(mod_03, newdata = predictor_control_ph_wave_j, type = 'probs')

predictions_treatment_ph_wave_j <- 
  predict(mod_03, newdata = predictor_treatment_ph_wave_j, type = 'probs')

predictions_predictors_control_ph_wave_j <- 
  bind_cols(
    predictor_control_ph_wave_j, predictions_control_ph_wave_j
  ) %>% 
  pivot_longer(cols = statuses, names_to = "current_status", values_to = "predicted_probability") %>% 
  mutate(arm = "control")
...
predictions_predictors_both_ph_wave_j <- 
  bind_rows(
    predictions_predictors_control_ph_wave_j,
    predictions_predictors_treatment_ph_wave_j
  )
...
```{r}
summary_estimates_wave_j_ph <- 
  predictions_predictors_both_ph_wave_j %>% 
    group_by(arm, current_status) %>% 
    summarise(predicted_total = sum(predicted_probability)) %>% 
    pivot_wider(names_from = arm, values_from = predicted_total) %>% 
    mutate(
      share_control = control / sum(control), 
      share_treatment = treatment / sum(treatment)
    ) %>% 
    select(current_status, control, share_control, treatment, share_treatment) %>% 
    ungroup()

write_csv(x = summary_estimates_wave_j_ph, file = "wave_j_summary_estimates_ph.csv")
summary_estimates_wave_j_ph
