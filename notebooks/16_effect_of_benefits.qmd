...
income_costs_by_hhtype %>% 
  ...
  labs(
    ...
    title = "Distribution of equivalised net monthly household incomes in UKHLS",
...
child_ben_uplift_scenario %>% 
  ...
  mutate(combined = paste(scenario, parenting_type, sep = "__")) %>% 
  select(-has_couple, -scenario, -parenting_type) %>% 
  pivot_wider(names_from = combined, values_from = share_in_poverty) %>% 
  select(wave, baseline__lone_parent, counterfactual__lone_parent, baseline__couple, counterfactual__couple)
```
